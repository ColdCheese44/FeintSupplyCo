...
- FeintSupplyCo sends `x-api-key` as `ETSY_API_KEY:ETSY_API_SECRET` and `Authorization: Bearer <token>` on authenticated calls.
...
FeintSupplyCo follows the Etsy draft-listing workflow described in Etsy's official listings tutorial:
...
- FeintSupplyCo stores raw research snapshots in SQLite so pricing and taxonomy hints can be reused later.

## Implementation notes

- Etsy taxonomy is inferred from recent research snapshots when possible.
- Many shops need additional shipping and readiness configuration for physical listings. FeintSupplyCo reads optional environment overrides for those values rather than hardcoding them.
